...
function listRegistry()
{
    ...
    $.when($.post('/composer/admin/getMembers', options)).done(function (_results)
    {
        ...
        _str +='<h2>Registry List</h2>';
        _str += '<h4>Network update results: '+_results.result+'</h4>';
        _str += '<table width="100%"><tr><th>Type</th><th>Company</th><th>username</th></tr>';
...
function listAssets()
{
    console.log("In list assets");
    let options = {};
    options.registry = 'Documents';
    options.type='admin';
    $.when($.post('/composer/admin/getAssets', options)).done(function (_results)
    {   
        console.log("from list assets");
        ...
        {
            _str += '<table width="100%"><tr><th>User Id</th><th>Aadhar Number</th></tr>';
            for (let each in _results.orders)
            {(function(_idx, _arr){
                _str += '<tr><td align="center">'+_arr[_idx].id+'</td><td>'+JSON.parse(_arr[_idx].value[0]).aadhar_number+'</td></tr>';
